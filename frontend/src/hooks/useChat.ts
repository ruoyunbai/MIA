...
export function useChat() {
    ...
    const handleSend = async (text?: string) => {
        ...
        if (!contentToSend.trim() || !activeConversation) return;
    ...
    }, [activeConversationId, conversations, deleteConversation, setActiveConversationId, addConversation]);

    const handleSourceClick = useCallback((source: typeof selectedSource) => {
        if (!source) return;
        if (selectedSource) {
    ...
    }, [selectedSource, sourceHistory, historyIndex]);
