...
export class LlmService {
  ...
  private initLlm() {
    ...
    const baseUrl = (
      miaBaseUrl || this.configService.get<string>('OPENAI_API_BASE_URL')
    )?.trim();
    ...
    const modelName = (
      miaModelName || this.configService.get<string>('OPENAI_MODEL_NAME')
    )?.trim();
    ...
    console.log(
      ...
      baseUrl,
      miaBaseUrl
        ? '(from MIA_OPENAI_API_BASE_URL)'
        : '(from OPENAI_API_BASE_URL)',
    );
    console.log(
      'Model:',
      modelName,
      miaModelName
        ? '(from MIA_OPENAI_MODEL_NAME)'
        : '(from OPENAI_MODEL_NAME)',
    );
    console.log(
      'API Key:',
      apiKey
        ...
        : 'undefined',
      miaApiKey ? '(from MIA_OPENAI_API_KEY)' : '(from OPENAI_API_KEY)',
    );
  ...
  async getVectorStore() {
    if (!this.vectorStore) {
      ...
      const miaEmbeddingModel = this.configService.get<string>(
        'MIA_OPENAI_EMBEDDING_MODEL',
      );
      const embeddingModel = (
        miaEmbeddingModel || 'text-embedding-ada-002'
      )?.trim();
